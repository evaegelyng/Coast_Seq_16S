...
libraries = [x for x in glob("/home/mpavila/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/base/data/*") if os.path.isdir(x)]
    ...
    with open(os.path.join(library_root, "tags.txt")) as tags_file:
        for line in tags_file:
            ...
            gwf.target(
                ...
                mkdir -p {folderAS}
                mkdir -p {folderSS}
                sickle se -l 160 -q 25 -x -t sanger -f {inputASF} -o {outputASF} || echo "Empty file " {outputASF} 
                sickle se -l 160 -q 25 -x -t sanger -f {inputASR} -o {outputASR} || echo "Empty file " {outputASR} 
                sickle se -l 160 -q 25 -x -t sanger -f {inputSSF} -o {outputSSF} || echo "Empty file " {outputSSF} 
                sickle se -l 160 -q 25 -x -t sanger -f {inputSSR} -o {outputSSR} || echo "Empty file " {outputSSR} 
...
#Fixing disordered/unmatching reads
for library_root in libraries:
    library_id = os.path.basename(library_root)
    ...
    with open(os.path.join(library_root, "tags.txt")) as tags_file:
        for line in tags_file:
            ...
            output_files.append("tmp/{}/DADA2_AS/filtered/fixed/{}_F_filtered_fixed.fastq".format(library_id, tag_id))
            output_files.append("tmp/{}/DADA2_AS/filtered/fixed/{}_R_filtered_fixed.fastq".format(library_id, tag_id))
            output_files.append("tmp/{}/DADA2_AS/filtered/fixed/{}_singleone.fastq".format(library_id, tag_id))  
            output_files.append("tmp/{}/DADA2_SS/filtered/fixed/{}_F_filtered_fixed.fastq".format(library_id, tag_id))
            output_files.append("tmp/{}/DADA2_SS/filtered/fixed/{}_R_filtered_fixed.fastq".format(library_id, tag_id))
            output_files.append("tmp/{}/DADA2_SS/filtered/fixed/{}_singleone.fastq".format(library_id, tag_id))
            
            folderAS="tmp/{}/DADA2_AS/filtered/fixed".format(library_id)
            folderSS="tmp/{}/DADA2_SS/filtered/fixed".format(library_id)            
            
            gwf.target(
                name="fixing_{}_{}_{}".format(project_name,library_id, tag_id),
                inputs=input_files,
                outputs=output_files,
                cores=1,
                memory="4g",
                ...
                mkdir -p {folderAS}
                mkdir -p {folderSS}
                repair.sh in1={inputASF} in2={inputASR} out1={outputASF} out2={outputASR} outs={outputAsing} || echo "Empty file " {outputASF} {outputASR}
                repair.sh in1={inputSSF} in2={inputSSR} out1={outputSSF} out2={outputSSR} outs={outputSsing} || echo "Empty file " {outputSSF} {outputSSR} 
                            """.format(folderAS=folderAS,folderSS=folderSS,inputASF=input_files[0],inputASR=input_files[1],inputSSF=input_files[2],inputSSR=input_files[3],outputASF=output_files[0],outputASR=output_files[1],outputAsing=output_files[2],outputSSF=output_files[3],outputSSR=output_files[4],outputSsing=output_files[5])
...
for library_root in libraries:
    ...
    with open(os.path.join(library_root, "tags.txt")) as tags_file:
        for line in tags_file:
            ...
            input_files.append("tmp/{}/DADA2_AS/filtered/fixed/{}_F_filtered_fixed.fastq".format(library_id, tag_id))
            input_files.append("tmp/{}/DADA2_AS/filtered/fixed/{}_R_filtered_fixed.fastq".format(library_id, tag_id))
            input_files.append("tmp/{}/DADA2_SS/filtered/fixed/{}_F_filtered_fixed.fastq".format(library_id, tag_id))
            input_files.append("tmp/{}/DADA2_SS/filtered/fixed/{}_R_filtered_fixed.fastq".format(library_id, tag_id))
