...
if ( file.info(inputFiles[3])$size == 0 || file.info(inputFiles[4])$size == 0  ) {
  ...
  print(paste(inputFiles[4], " has size ", file.info(inputFiles[4])$size)) } else {
   ...
   fastqPairedFilter(c(inputFiles[3],inputFiles[4]), c(outputFiles[3],outputFiles[4]), minLen=160, maxN=0, maxEE=2, truncQ=2,
                     matchIDs=TRUE)
   print("done")
}
...
if ( file.info(inputFiles[1])$size == 0 || file.info(inputFiles[2])$size == 0  ) {
  ...
  print(paste(inputFiles[2], " has size ", file.info(inputFiles[2])$size)) } else {
   ...
   fastqPairedFilter(c(inputFiles[1],inputFiles[2]), c(outputFiles[1],outputFiles[2]), minLen=160, maxN=0, maxEE=2, truncQ=2,
                     matchIDs=TRUE)
   print("done")
}
