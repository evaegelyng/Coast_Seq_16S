...
otu_mat<-as.matrix(read.table("cleaned_otu_table_ASV_wise_EES.txt", sep="\t", header=T, row.names=1,check.names=F))
...
metadata<-read.table("merged_seq_run_cleaned_metadata_ASV_wise_EES.txt", sep="\t", header=T)
...
sum(sample_sums(pert)) # 432775904
cat("\n")
cat("example_sample_with_singleton_seq94_2C10EB1_2_before_removal")
new_otu_mat["seq94","2C10EB1_2"]
...
cat("\n")
cat("example_sample_with_singleton_seq94_2C10EB1_2_after_removal")
new_otu_mat["seq94","2C10EB1_2"]
...
setwd("/home/evaes/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/results")
write.table(shablaw, "no_sing_cleaned_otu_table_ASV_wise_EES.txt", sep="\t", quote=FALSE, row.names=TRUE)

setwd("/home/evaes/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/results/metadata")
write.table(data.frame(sample_data(tsa)), "no_control_no_sing_samples_cleaned_metadata_ASV_wise_EES.txt", sep="\t", quote=FALSE, row.names=TRUE)