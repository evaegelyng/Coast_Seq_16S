...
setwd("/home/mpavila/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/base/results")

#import data with check.names=F
otu_mat<-read.table("DADA2_nochim.table", sep="\t", header=T, row.names=1,check.names=F,colClasses="character")

setwd("/home/mpavila/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/results/taxonomy_long") 

taxonomy_silva<-read.table("taxonomy.tsv", sep='\t', header=T, comment="")
ttt_silva<-strsplit(as.character(taxonomy_silva$Taxon), ";")
n_silva <- max(sapply(ttt_silva, length))
l_silva <- lapply(ttt_silva, function(X) c(X, rep(NA, n_silva - length(X))))
tax_table_silva<-data.frame(t(do.call(cbind, l_silva)))
colnames(tax_table_silva)<-c("Kingdom","Phylum","Class","Order","Family","Genus")
rownames(tax_table_silva)<-taxonomy_silva$Feature.ID
tax_table_silva$conf<-taxonomy_silva$Confidence

nrow(tax_table_silva)
new_tax<-subset(tax_table_silva, conf>=0.7)
nrow(new_tax)
new_tax2<-new_tax[!is.na(new_tax$Family),]
nrow(new_tax2)
tax_mat_silva<-as.matrix(new_tax2)
...
setwd("/home/mpavila/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/results/metadata")
write.table(test_md, "metadata_both.txt", sep="\t", row.names = T, quote=FALSE)
...
write.table(f_otu_mat, "otu_silva.txt", sep="\t", quote=FALSE, row.names=TRUE)
write.table(tax_mat_b, "tax_silva.txt", sep="\t", quote=FALSE, row.names=TRUE)
