# Run from both_silva/results folder
# conda activate hmsc
...
otu_mat<-as.matrix(read.table("otu_silva_dada2.txt", sep="\t", header=T, row.names=1,check.names=F))
...
p_metadata<-read.table("metadata/metadata_both_dada2.txt", sep="\t", header=T)
...
final_table_s<-merge_samples(sddd, "sr_PCR_r", fun=sum)
final_table_w<-merge_samples(wddd, "sr_PCR_r", fun=sum)

new_s_otu<-as.matrix(data.frame(otu_table(final_table_s), check.names=F))
new_w_otu<-as.matrix(data.frame(otu_table(final_table_w), check.names=F))
...
as.character("CNE"), ifelse(metadata$p_source=="control", as.character("control"), ifelse(grepl("Ns", metadata$ppn, fixed=T)|grepl("SN", metadata$ppn, fixed=T)|grepl("Nw", metadata$ppn, fixed=T)|grepl("WN", metadata$ppn, fixed=T), as.character("NTC"), as.character("Field_sample"))))
...
extraction_refs<-read.table("metadata/extraction_refs_both_seasons.txt", sep="\t", header=T, row.names=1)
...
PSU_refs<-read.table("metadata/PSU_refs_both.txt", sep="\t", header=T, row.names=1)
...
setwd("/home/evaes/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/results/cleanup_ASV_wise_EES/spring")
...
setwd("/home/evaes/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/tmp_EES")
saveRDS(mandss, "mandss.rds")
...
setwd("/home/evaes/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/results/cleanup_ASV_wise_EES/autumn")
...
write.table(utus, "cleaned_otu_table_ASV_wise_EES.txt", sep="\t", quote=FALSE, row.names=TRUE)
...
write.table(data.frame(sample_data(final_table2), check.names=F), "merged_seq_run_cleaned_metadata_ASV_wise_EES.txt", sep="\t", quote=FALSE, row.names=TRUE)

###Summarize and write new ASV table

setwd("/home/evaes/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/results/cleanup_ASV_wise_EES/summary")
...
setwd("/home/evaes/eDNA/faststorage/Velux/CoastSequence/Autumn/Bac16s/both_silva/results")
otu_matt<-as.matrix(read.table("cleaned_otu_table_ASV_wise_EES.txt", sep="\t", header=T, row.names=1,check.names=F))
