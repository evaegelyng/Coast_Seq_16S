...
function(lib,cores,d=13,min_reads_MOTU=2,min_reads_ESV=2,run_swarm=TRUE,generate_ESV=FALSE,obipath=""){
  ...
  if (run_swarm){
    ...
    system(paste0("swarm -d ",d," -z -t ",cores," -o ",lib,"_SWARM_output -s ",lib,"_SWARM",d,"nc_stats -w ",lib,"_SWARM_seeds.fasta ",lib_in,"COSQ_16S.fa"),intern=T,wait=T)
    message("ODIN will recount abundances for every MOTU after Swarm.")
  }
  fileswarm=paste0(lib,"_SWARM_output")
  filetab=paste0(lib_in,"DADA2_nochim.table")
...
mjolnir4_ODIN_eva(lib,cores,d=6,run_swarm=TRUE,generate_ESV=FALSE)
