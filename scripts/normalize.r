...
# Should be run from the both_seasons/results folder, using the hmsc environment
...
###Load OTU table
otu_mat<-as.matrix(read.table("cleaned_otu_silva_EES3.txt", sep="\t", header=T, row.names=1,check.names=F))
...
###Load taxonomy table
taxonomy_silva<-read.table("cleaned_tax_silva_EES3.txt", sep='\t', header=T, comment="")
...
###Load metadata
metadata<-read.table("metadata/cleaned_silva_metadata_EES3.txt", sep="\t", header=T)
...
COSQ_final = merge_phyloseq(p_SILVA, sampledata)
COSQ_final
